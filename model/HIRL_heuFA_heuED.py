...
class HIRL_heuFA_heuED(nn.Module):
    def __init__(self, config, params, user_profile, item_profile):
        super(HIRL_heuFA_heuED, self).__init__()
    ...
    def train(self, training_domains, valiation_domain):
        # In this section, we want to obtain the most important field(feature) to allocate the environment.
        # And calculate the environment prob.(float one-hot) of each interaction.
        training_set = []
        for did, d in enumerate(training_domains):
            ...
            for (uid, iid, y) in d:
                ...
                x_list_classifier.append(x)
                y_list_classifier.append(y)
            x_list_classifier = torch.tensor(x_list_classifier)
            y_list_classifier = torch.tensor(y_list_classifier)

            tree_classifier = DecisionTreeClassifier()
            tree_classifier.fit(x_list_classifier, y_list_classifier)
            important_field_ind = np.argmax(tree_classifier.feature_importances_)
            # print(important_field_ind)
            important_field_values = x_list_classifier[:, important_field_ind]
            max_field_value, min_field_value = max(important_field_values), min(important_field_values)
            important_field_values = (important_field_values - min_field_value) / (max_field_value - min_field_value)
            environment_threshold = 1.0 / (self.config['num_source_domain'] - 1) + 0.000001
            important_field_tag = torch.floor(important_field_values / environment_threshold).long()
            # print(important_field_tag,max(important_field_tag),min(important_field_tag))
            important_field_tag_one_hot = torch.nn.functional.one_hot(important_field_tag).detach().float().tolist()

            for ind, (uid, iid, y) in enumerate(d):
                training_set.append((uid, iid, y, did, important_field_tag_one_hot[ind]))
        ...
        for e in range(self.epoch):
            ...
            for step, (x, y, d, p) in enumerate(training_dataloader):
                ...
                # - Prediction score
                tilde_p_selected = copy(p_selected)

                # Change the best and second prob.
                # print(environment_prob)
                val, ind = torch.topk(tilde_p_selected, k=2, dim=-1)
                best_ind, second_ind = ind[:, 0], ind[:, 1]
                # print(best_ind,second_ind)
                for jnd in range(tilde_p_selected.shape[0]):
                    tilde_p_selected[jnd][best_ind[jnd]] = val[jnd][1]
                    tilde_p_selected[jnd][second_ind[jnd]] = val[jnd][0]
                ...
                loss_domain_reconstruct = torch.nn.functional.mse_loss(domain_reconstruct_embedding,
                                                                       domain_inv_embedding)
                loss_environment_classify = torch.nn.functional.mse_loss(tilde_p_selected,
                                                                         torch.softmax(
                                                                             self.classifier_box.classifier1_component(
                                                                                 environment_var_embedding), dim=-1)) - \
                                            torch.nn.functional.mse_loss(tilde_p_selected,
                                                                         torch.softmax(
                                                                             self.classifier_box.classifier1_component(
