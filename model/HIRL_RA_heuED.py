...
class HIRL_RA_heuED(nn.Module):
    def __init__(self, config, params, user_profile, item_profile):
        super(HIRL_RA_heuED, self).__init__()
    ...
    def train(self, training_domains, valiation_domain):
        ...
        for e in range(self.epoch):
            ...
            for step, (x, y, d) in enumerate(training_dataloader):
                ...
                environment_prob = self.environment_component.environment_prob(environment_score)
                tilde_environment_prob = copy(environment_prob.detach())

                # Change the best and second prob.
                # print(environment_prob)
                val, ind = torch.topk(tilde_environment_prob, k=2, dim=-1)
                best_ind, second_ind = ind[:, 0], ind[:, 1]
                # print(best_ind,second_ind)
                for jnd in range(tilde_environment_prob.shape[0]):
                    tilde_environment_prob[jnd][best_ind[jnd]] = val[jnd][1]
                    tilde_environment_prob[jnd][second_ind[jnd]] = val[jnd][0]
                # print(environment_prob)

                entropy = torch.sum(torch.multiply(tilde_environment_prob, torch.log2(environment_prob)), dim=-1)
