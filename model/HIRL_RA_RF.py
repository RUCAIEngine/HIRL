    ...
    def train(self, training_domains, valiation_domain):
        ...
        for e in range(self.epoch):
            ...
            for step, (x, y, d) in enumerate(training_dataloader):
                ...
                # Step 1: Refine soft environment probabilities.

                # - Calculate the noise component.
                x_with_grad = torch.tensor(x, requires_grad=True)
                embedding_with_grad = self.invariant_component.embedding(x_with_grad)

                environment_mask_with_grad = self.invariant_component.environment_mask(embedding_with_grad)
                environment_var_embedding_with_grad = self.invariant_component.environment_var_embedding(
                    embedding_with_grad, environment_mask_with_grad)

                environment_score_with_grad = self.environment_component.environment_score(embedding_with_grad, d)
                environment_prob_with_grad = self.environment_component.environment_prob(environment_score_with_grad)
                environment_combination_retrieval_with_grad = self.environment_component.environment_combination_retrieval(
                    environment_prob_with_grad, d)

                entropy_with_grad = torch.sum(
                    torch.multiply(environment_prob_with_grad, torch.log2(environment_prob_with_grad)), dim=-1)
                entropy_with_grad = torch.mean(entropy_with_grad)

                loss = torch.nn.functional.mse_loss(environment_var_embedding_with_grad,
                                                    environment_combination_retrieval_with_grad) + \
                       self.params['environment_assign_loss_alpha'] * entropy_with_grad

                disturb_grad = torch.autograd.grad(loss, x_with_grad)[0]
                noise_component = self.params['environment_refine_noise_bound'] / np.sqrt(
                    self.embedding_dim) * disturb_grad

                # - Calculate the loss.
                noise_x = x + noise_component
                embedding = self.invariant_component.embedding(noise_x)
                ...
                # Step 2: Re-optimize invariant components.
                embedding = self.invariant_component.embedding(x)
